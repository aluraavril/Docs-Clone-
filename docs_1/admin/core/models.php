...
function saveUserImage($user_id, $image_binary, $extension)
{
    ...
    $upload_dir = __DIR__ . "/../user/img/$user_id/";
    ...
    return "user/img/$user_id/$filename";
}
...
{
    $stmt = $pdo->query("
        SELECT d.document_id, d.title, d.last_modified, u.username AS author_name
        FROM documents d
        JOIN users u ON d.author_id = u.user_id
...
{
    $stmt = $pdo->prepare("SELECT user_id, username, email, date_created, is_suspended, suspend_comment FROM users WHERE is_admin = 0");
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}


function toggleSuspendUser($pdo, $user_id, $is_suspended)
{
...
function updateSuspendComment(PDO $pdo, int $user_id, string $comment): bool
{
    $sql = "UPDATE users SET suspend_comment = :comment WHERE user_id = :user_id";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute(['comment' => $comment, 'user_id' => $user_id]);
}
